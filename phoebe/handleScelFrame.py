    ...
    for w in wordList:
        ...
        if os.path.isfile(filePath):
            ...
            if ' '+wi[0] in tagDic.keys():
                ...
                tagDic[' '+wi[0]]=[wi[1]]
            f=open(filePath,'w')
            f.write('\n'.join([k+':'+','.join(tagDic[k]) for k in sorted(tagDic.keys())]))
            f.close()
        else:
            f=open(filePath,'w')
            f.write(' '+wi[0]+':'+wi[1])
    ...
    for t in tList:
        ...
        for w in pattern.findall(sent):
            ...
            for i in range(len(wL)-1):
                ...
                text=''
                try:
                    f=open(os.path.abspath('.')+'\\RoleDicFre\\'+wi+'.txt','r')
                    text=f.read()
                    f.close()
                except:
                    None
        ...
        for w in pattern.findall(sent):
            ...
            for i in range(len(wL)-1):
                ...
                if word in tDic.keys():
                    ...
                    tDic[word]=1
        f=open(filePath,'w')
        f.write('\n'.join([k+':'+str(tDic[k]) for k in sorted(tDic.keys())]))
        f.close()
...
def MorphemeRoleDic(sent):
    ...
    f.write('\n'.join([k+':'+','.join(RoleDic[k]) for k in sorted(RoleDic.keys())]))
    f.close()
    ...
    for k in wordDic.keys():
        ...
        if not os.path.isfile(filePath):
            f=open(filePath,'w')
            f.write('\n'.join([ki+':'+str(wordDic[k][ki]) for ki in wordDic[k].keys()]))
            f.close()
            None
        else:
            f=open(filePath,'r')
...
class handleScelFrame(wx.Frame):
    def __init__(self):
        ...
        self.sentList=sorted([f for f in os.listdir(os.path.abspath('.')+'\\special\\') if f])
        self.Index=int(re.compile('\S+').findall(open(os.path.abspath('.')+'\\handleIndex.txt','r').read())[0])%len(self.sentList)
        self.panel=wx.Panel(self,-1)
        self.sizer=wx.FlexGridSizer(cols=2,hgap=10,vgap=10)
        label1=wx.StaticText(self.panel,-1,u'待处理文本')
        self.text1=wx.TextCtrl(self.panel,-1,self.sentList[0],size=(350,20))
        label2=wx.StaticText(self.panel,-1,u'已处理文本')
        self.text2=wx.TextCtrl(self.panel,-1,'',size=(350,300),style=wx.TE_MULTILINE)
        ...
        self.sizer.AddMany([label1,self.text1,label2,self.text2,self.button1,self.button2,
    ...
    def OnButton1(self,event):
        ...
        self.text2.SetValue('\n'.join(sorted(set([f[2]+'/' for f in handleScel.GTable]))))
    ...
    def OnNr(self,event):
        wordList=[w+'/nr' for w in re.compile(r'\S+(?=/)').findall(self.text2.GetValue())]
        self.text2.SetValue('\n'.join(wordList))
    def OnNt(self,event):
        wordList=[w+'/nt' for w in re.compile(r'\S+(?=/)').findall(self.text2.GetValue())]
        self.text2.SetValue('\n'.join(wordList))
    def OnNs(self,event):
        wordList=[w+'/ns' for w in re.compile(r'\S+(?=/)').findall(self.text2.GetValue())]
        self.text2.SetValue('\n'.join(wordList))
    def OnNz(self,event):
        wordList=[w+'/nz' for w in re.compile(r'\S+(?=/)').findall(self.text2.GetValue())]
        self.text2.SetValue('\n'.join(wordList))

class handleScelFrame2(wx.MDIParentFrame):
    def __init__(self):
        global Index
        wx.MDIParentFrame.__init__(self,None,-1,u'handleScel测试版02',size=(500,500),style=wx.DEFAULT_FRAME_STYLE|wx.VSCROLL)
...
class handleScelFrame3(wx.Frame):
    def __init__(self):
        ...
        self.dicPath2=os.path.abspath('.')+u'\\城市信息精选部分标注结果\\'.encode('gb2312')
        self.dicPath3=os.path.abspath('.')+u'\\城市信息精选部分完结\\'.encode('gb2312')
        if not os.path.isdir(self.dicPath2):
            os.mkdir(self.dicPath2)
        ...
        if not self.sentList:
            self.panel=wx.Panel(self,-1)
            self.text=wx.StaticText(self.panel,-1,u'The file path do not exist or do not has any subpath')
            return
        self.Index=0#int(re.compile('\S+').findall(open(os.path.abspath('.')+'\\handleIndex.txt','r').read())[0])%len(self.sentList)
        self.panel=wx.Panel(self,-1)
        self.sizer=wx.FlexGridSizer(cols=2,hgap=10,vgap=10)
    ...
    def OnButton1(self,event):
        ...
        try:
            ...
            f.close()
            self.text2.SetValue('\n'.join([ONRecognition(s)[0] for s in sentList]))
        except:
...
def GetSRTZ2():
    ...
    for t in ['nr','nt','ns','nz']:
        ...
        f.write('\n'.join(sorted(set([s[GetSeparattion1(s)[-2]:GetSeparattion1(s)[-1]] for s in sorted(set(pattern.findall(text)))]))))
        f.close()
...
def BatchTest(path1,path2):
    ...
    for f in fileList2:
        ...
        if f[:-4] in fileList1:
            ...
            SumFLen+=1
    if SumFLen:
        file3.write('\n\n\nSumR:'+str(SumR/SumFLen)+'   SumT:'+str(SumT/SumFLen)+'\n')
    file3.close()
                ...
                if t[0] in tList:
                    ...
                    if not os.path.isdir(dicPath):
                        ...
                        if os.path.isfile(dicPath+f):
                            wList=re.compile(r'\S+').findall(open(dicPath+f,'r').read())
...
def MiniEdiTag(w):
    ...
    for f in os.listdir(dicPath):
        ...
        if miniDis in tagDic.keys():
            if abs(len(miniW)-len(w))<abs(len(tagDic[miniDis].split(':')[1])-len(w)):
                tagDic[miniDis]='/'+f+':'+miniW+':'+str(miniDis)
            elif abs(len(miniW)-len(w))==abs(len(tagDic[miniDis].split(':')[1])-len(w)):
