...
#print numList'''
numList = [
    4408, 8887, 6207, 1828, 2876, 1869, 2596, 8976, 7089, 1112, 3610, 8540,
    125, 2023, 6388, 1868, 8822, 4051, 3609, 8705, 821, 8404, 8004, 2002, 4490,
    4237, 6948, 5779, 5511, 3873, 5073, 6508, 1944, 5312, 1667, 5324, 446,
    9585, 6865, 6078, 2770, 9145, 9683, 6289, 7850, 1622, 1424, 4482, 6655,
    5958, 9356, 8026, 4007, 8976, 9692, 7122, 1045, 6155, 5847, 4133, 5716,
    1483, 8782, 6482, 7718, 2631, 3799, 8654, 662, 2346, 8986, 8612, 1514,
    3307, 4526, 6261, 4554, 1831, 1409, 7353, 4254, 9144, 49, 2418, 138, 8784,
    6144, 4516, 7185, 4749, 7792, 7168, 2545, 345, 167, 7724, 2461, 6346, 4834,
    9534
]
            ...
            for i in range(len(numList) - E, len(numList) - B):
                if flag:
                    if numList[-i - 1] > numList[-i]:
                        numList[-i -
                                1], numList[-i] = numList[-i], numList[-i - 1]
                else:
                    if numList[-i - 1] < numList[-i]:
                        numList[-i -
                                1], numList[-i] = numList[-i], numList[-i - 1]
    ...
    while len(sortedNumList) > 1:
        ...
        for i in range(len(sortedNumList))[::2]:
            if i < len(sortedNumList) - 1:
                ...
                index1 = 0
                index2 = 0
                while index1 < len(sortedNumList[i]) and index2 < len(
                        sortedNumList[i + 1]):
                    if flag:
                        if sortedNumList[i][index1] < sortedNumList[i +
                                                                    1][index2]:
    ...
    while index < len(sortedNumList):
        if type(sortedNumList[index]) == list:
            if sortedNumList[index]:
                sortedNumList = sortedNumList[:index] + [
                    sortedNumList[index][1], sortedNumList[index][0],
                    sortedNumList[index][2]
                ] + sortedNumList[index + 1:]
                #index+=1
            else:
                sortedNumList = sortedNumList[:index] + sortedNumList[index +
                                                                      1:]
        else:
            index += 1
    return sortedNumList
