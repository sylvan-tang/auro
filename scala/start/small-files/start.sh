...
# read auro/README.md for more information
current_path=$(pwd)
...
# shellcheck disable=SC2046
auro_path=$(dirname $(dirname "$shell_path"))

# shellcheck disable=SC2164
cd "$auro_path"

sbt auroSmallFiles:assembly
...
#
#  spark-submit \
#    --class com.sylvan.auro.bigdata.files.Main \
#    --executor-memory 2G \
#    --driver-memory 4G \
#    ${shell_path}/target/scala-2.11/auro-small-files-assembly-0.1.0-SNAPSHOT.jar "$folder_path" "$data_num" "$file_num" -1 > ${log_path} 2>&1
...
#  --executor-memory 2G \
#  --driver-memory 4G \
#  ${shell_path}/target/scala-2.11/auro-small-files-assembly-0.1.0-SNAPSHOT.jar "$folder_path" 200000 100000 "$i" > ${log_path} 2>&1
#  grep "cost" ${log_path} >> "$output_path"
#done
