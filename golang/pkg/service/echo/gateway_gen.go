...
func RunGateway(ctx context.Context) {
	serverConfig := customgrpc.GetGatewayServerConfig(ctx, getPkgName())
	ctrl := goproc.NewController(ctx, customgrpc.GatewayServerKey)
	defer ctrl.Wait()
	ctrl.WithValue("config", serverConfig).Go(ServeGateway)
}
...
func ServeGateway(ctx context.Context) {
	...
	g.Log().Infof("starting gateway server: name=%s address=%s grpc_endpoint=%s", cfg.Name, cfg.Address(), cfg.GrpcAddress())
	if err := echov1.RegisterGreetingServiceHandlerFromEndpoint(ctx, mux, cfg.GrpcAddress(), opts); err != nil {
		g.Log().Fatalf("failed to register echov1.RegisterGreetingServiceHandlerFromEndpoint handler: name=%s error=%s", cfg.Name, err)
	}
