...
func ServeGateway(ctx context.Context) {
	...
	if err := assocv1.RegisterAssocServiceHandlerFromEndpoint(ctx, mux, cfg.GrpcAddress(), opts); err != nil {
		g.Log().Fatalf("failed to register assocv1.RegisterAssocServiceHandlerFromEndpoint handler: name=%s error=%s", cfg.Name, err)
	}
