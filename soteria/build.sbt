...
ThisBuild / organization := "com.sylvan.soteria"
...
// 具体的启动类
lazy val soteriaSmallFiles = (project in file("./start/small-files"))
  .settings(commonSettings: _*)
  .dependsOn(soteriaTool)
  .settings(
    name := "soteria-small-files"
  )


// 书籍相关分享
lazy val soteriaBook = (project in file("./soteria-book"))
  .settings(commonSettings: _*)
  .settings(
    name := "soteria-book"
  )
...
// 职业生涯相关分享
lazy val soteriaCareer = (project in file("./soteria-career"))
  .settings(commonSettings: _*)
  .settings(
    name := "soteria-career"
  )
...
// 基础语法分享
lazy val soteriaCommon = (project in file("./soteria-common"))
  .settings(commonSettings: _*)
  .settings(
    name := "soteria-common"
  )
...
// 金融分享
lazy val soteriaFinancial = (project in file("./soteria-financial"))
  .settings(commonSettings: _*)
  .dependsOn(soteriaCommon)
  .settings(
    name := "soteria-financial"
  )

// 公共第三方框架
lazy val soteriaInfrastructure = (project in file("./soteria-infrastructure"))
  .settings(commonSettings: _*)
  .dependsOn(soteriaCommon)
  .settings(
    name := "soteria-infrastructure"
  )

// 第三方框架分享
lazy val soteriaTool = (project in file("./soteria-tool"))
  .settings(commonSettings: _*)
  .settings(libraryDependencies ++= sparkSeq ++ elasticSeq)
  .dependsOn(soteriaCommon)
  .dependsOn(soteriaInfrastructure)
  .settings(
    name := "soteria-tool"
  )
