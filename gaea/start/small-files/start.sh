...
# read gaea/README.md for more information
current_path=$(pwd)
# shellcheck disable=SC2046
# shellcheck disable=SC2164
shell_path=$(cd $(dirname "${BASH_SOURCE[0]}"); pwd )
# shellcheck disable=SC2046
gaea_path=$(dirname $(dirname "$shell_path"))
...
#
#  spark-submit \
#    --class com.sylvan.gaea.bigdata.files.Main \
#    --executor-memory 2G \
#    --driver-memory 4G \
#    ${shell_path}/target/scala-2.11/gaea-small-files-assembly-0.1.0-SNAPSHOT.jar "$folder_path" "$data_num" "$file_num" -1 > ${log_path} 2>&1
...
#  --executor-memory 2G \
#  --driver-memory 4G \
#  ${shell_path}/target/scala-2.11/gaea-small-files-assembly-0.1.0-SNAPSHOT.jar "$folder_path" 200000 100000 "$i" > ${log_path} 2>&1
#  grep "cost" ${log_path} >> "$output_path"
#done
