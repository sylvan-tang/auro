...
const (
	header = "// Code generated by github.com/sylvan/auro/toolkits/cmd/gen-proto. DO NOT EDIT.\n"
)
...
var (
	// Example:
	// go run "github.com/sylvan/auro/toolkits/cmd/gen-proto" --host=github.com/sylvan/auro/toolkits -f echo/v1 -f echo/v2
	opts struct {
		GoPackageModule string   `required:"true" long:"pkg" description:"Go option package module for whole proto." json:"GoPackageModule"`
...
func main() {
	...
	for _, file := range templates.Templates() {
		if file.Name() == "types.proto.tmpl" {
			if len(optList) > 0 {
				oneOpt := &Opt{EntityHeaders: strings.Join(
					[]string{
						"syntax = \"proto3\";",
						fmt.Sprintf("option go_package = \"%s/common/v1;commonv1\";", opts.GoPackageModule),
					}, "\n",
				)}
				err = os.MkdirAll(path.Join(rootPath, "common", "v1"), os.ModePerm)
				checkErr(err)
				output := path.Join(rootPath, "common", "v1", file.Name()[:len(file.Name())-5])
				...
				func() {
					...
					checkErr(err)
					err = file.Execute(f, &oneOpt)
					checkErr(err)
				}()
			}
			continue
		}
		fmt.Println("templates:", file.Name())
